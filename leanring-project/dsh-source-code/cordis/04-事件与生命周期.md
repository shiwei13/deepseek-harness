...
本节的[可运行代码](../../cordis-demos/04-events-and-lifecycle/01-emit/)位于独立目录中。先从仓库根目录进入该目录：

```sh
cd leanring-project/cordis-demos/04-events-and-lifecycle/01-emit
```
...
waterfall 可以把多个监听器组成一条环绕调用链。每个监听器拿到原始参数和 `next()`；调用 `next()` 才会继续执行下游。本节的[可运行代码](../../cordis-demos/04-events-and-lifecycle/02-waterfall/)也使用独立目录，请从仓库根目录进入：

```sh
cd leanring-project/cordis-demos/04-events-and-lifecycle/02-waterfall
```

下面例子先让一个监听器把下游结果变成大写，第二个监听器在输入包含 `blocked` 时接管结果：
...
本节的[完整可运行版本](../../cordis-demos/04-events-and-lifecycle/03-effect/lifecycle.ts)会在 700 毫秒后主动卸载子 Fiber，以便观察 disposer 清理定时器。请从仓库根目录运行：

```sh
cd leanring-project/cordis-demos/04-events-and-lifecycle/03-effect
node --import tsx ../../../../vendor/cordis/bin.js
```
